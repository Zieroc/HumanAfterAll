...
namespace HumanAfterAll
{
    class Switch : Tile
    {
        ...
        public List<Tile> _tilesToBeToggled = new List<Tile>();
        public Switch(Texture2D _texture, Vector2 _position, ContentManager _content, World _world, Player _player,TileManager _manager,Color _colour)
        {
        ...
        {
            foreach (FloorTile item in _tilesToBeToggled)
            {
                item._colour = _colour;
            }
            _body.Friction = 1;
        ...
        public override bool Body_OnCollision(Fixture fixtureA, Fixture fixtureB, FarseerPhysics.Dynamics.Contacts.Contact contact)
        {
            ...
            if (fixtureB.Body == _player._body)
            {
                foreach (Tile item in _tilesToBeToggled)
                {
                    item._body.Enabled = false;
                   
                    _manager.TilesInGame.Remove(item);
